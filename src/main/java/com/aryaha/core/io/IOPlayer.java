...
package com.aryaha.core.io;

import com.aryaha.core.game.commands.ICommand;
import com.aryaha.tictactoe.commands.CommandFactory;
...
public class IOPlayer implements Runnable {
     ...
     */
    public ICommand getBotCommand(long timeout) throws Exception {
        ...
        ICommand response = CommandFactory.fromJsonString(this.response);
        this.response = null;
        return response;
    }

    private void throwOnMaxErrorReached() throws Exception {
        if (this.errorCounter > this.MAX_ERRORS) {
            throw new Exception("player reached maximum errors");
        }
    }
